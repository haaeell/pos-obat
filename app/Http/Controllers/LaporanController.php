...
class LaporanController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('laporan.index', compact('ringkasan', 'saldo', 'dari', 'sampai'));
    }
    ...
    public function cetak(Request $request)
    {
        ...
        $pdf = Pdf::loadView('laporan.cetak', compact('ringkasan', 'saldo', 'dari', 'sampai', 'toko'))->setPaper('a4', 'portrait');
    ...
    {
        // Data transaksi periode laporan
        ...
        $hutangJatuhTempo = $hutangSupplier->filter(fn($b) => $b->tanggal_jatuh_tempo && $b->tanggal_jatuh_tempo->isPast())->count();
        ...
        $totalBeliBarang = BarangMasukDetail::whereHas(
            'barangMasuk',
            fn($q) => $q->where('jenis', 'masuk_normal')
        )->sum('subtotal');
